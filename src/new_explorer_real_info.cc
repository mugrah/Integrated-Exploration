...
//        myfile.close();
        ...
        try{
            listener2.waitForTransform(map_topic, base_link_topic, ros::Time(0), ros::Duration(5.0));
            listener2.lookupTransform(map_topic, base_link_topic, ros::Time(0), transform2);
        ...
        try{
            listener.waitForTransform(map_topic, odom_topic, ros::Time(0), ros::Duration(5.0));
            listener.lookupTransform(map_topic, odom_topic, ros::Time(0), transform);
        }catch (tf::TransformException ex){
            ROS_ERROR("%s",ex.what());
...
//        goal.pose.position.x = x + (((map_width-goal_cell.x)*map_cell)+map_x) - transform.getOrigin().x();
//        goal.pose.position.y = y + (((map_height-goal_cell.y)*map_cell)+map_y) - transform.getOrigin().y();
...
//        if(goal_cell.x<=cx){
//            int px = cx - goal_cell.x;
//            goal.pose.position.y = px*map_x/cx;
//        }else{
//            int px = goal_cell.x - cx;
//            goal.pose.position.y = -px*map_x/cx;
//        }

//        if(goal_cell.y<=cy){
//            int py = cy - goal_cell.y;
//            goal.pose.position.x = py*map_y/cy;
//        }else{
//            int py = goal_cell.y - cy;
//            goal.pose.position.x = -py*map_y/cy;
//        }

        goal.pose.position.x = map_x + goal_cell.x*map_cell;
        goal.pose.position.y = map_y - (goal_cell.y-map_height)*map_cell;
        ...
        int yr = map_height - ((actual_pose.pose.pose.position.y - map_y)/map_cell);
        int xr = (actual_pose.pose.pose.position.x - map_x)/map_cell;
        myfile.open (filename.c_str(),  std::ios::out | std::ios::app );
        myfile <<"odometria do robo  "<< x <<"  "<<y<<"\n";
        myfile <<"pose do robo  "<< xr <<"  "<<yr<<"\n";
        myfile <<"transformada da odometria  "<< transform.getOrigin().x() <<"  "<<transform.getOrigin().y()<<"\n";
        myfile <<"transformada do base_link  "<< transform2.getOrigin().x() <<"  "<<transform2.getOrigin().y()<<"\n";
        myfile <<"odom do goal  "<< goal.pose.position.x << " "<<goal.pose.position.y<<"\n";
        myfile <<"pose do goal  "<<goal_cell.x<<" "<<goal_cell.y<<"\n";
        myfile <<"tamanho do mapa  "<<map_width<<" "<<map_height<<"\n";
        myfile <<"origem da odometria  "<<map_x<<" "<<map_y<<"\n";
        myfile<<"----------------------------------------------------------------\n";
        myfile.close();
...
void ros_save_occ_map_Callback(gmapping::occMap occ_map){
    ...
    sprintf ( number, "%d", cont_map++ );

    std::ofstream myfile;
    std::string filename = "/home/rcolares/catkin_ws/src/ros-pioneer3at/maps/map_";//+number+".txt";
    filename+= number;
    filename+=".txt";
    myfile.open (filename.c_str(),  std::ios::out | std::ios::app );
    for(int i=0; i<height;i++){
        for(int e=0;e<width;e++){
            int t = (height - i -1)*width +e;
            myfile<<occ_map.data[t];
        }
    }
    myfile.close();
...
int main( int argc, char* argv[] )
{
    ...
    n_.getParam("odom_topic", odom_topic);
