...
{
    std::ofstream myfile;
    std::string filename = "/home/rcolares/catkin_ws/src/ros-pioneer3at/maps/odom"+robot_topic+".txt";
    myfile.open (filename.c_str(),  std::ios::out | std::ios::app );
    int y_odom = map_height - ((odometry.pose.pose.position.y - map_y)/map_cell);
    int x_odom = (odometry.pose.pose.position.x - map_x)/map_cell;
    ...
    myfile << odometry.pose.pose.position.x <<" "<< odometry.pose.pose.position.y<<" "<<MYaw<<" "<<x_odom<<" "<<y_odom<<"\n";
    myfile.close();
