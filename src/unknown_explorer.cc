...
ros::Publisher unknown_cmd_vel;
ros::Subscriber unknown_occ_map;
ros::Subscriber unknown_pose;
ros::Subscriber unknown_goal_status;
...
//     goal.goal_id.id = num_id;
    ...
    goal.goal.target_pose.pose.position.x = x + ((frontier_vector[chosen_cell].x_mean*map_cell)+map_x) - transform.getOrigin().x();
    goal.goal.target_pose.pose.position.y = y + (((map_height-frontier_vector[chosen_cell].y_mean)*map_cell)+map_y) - transform.getOrigin().y();
//     if(robot_topic[5] == '1'){
...
void ros_save_occ_map_Callback(gmapping::occMap occ_map){
  ...
  if(goal_plan==0){
    ...
    chosen_cell = unknown_information(real_map, width, height);
...
//   frontier_cmd_vel = n.advertise<geometry_msgs::PoseStamped>(goal_topic, 1);
  unknown_cmd_vel = n.advertise<move_base_msgs::MoveBaseActionGoal>(goal_topic, 1);
  
  unknown_occ_map = n.subscribe(occ_map_topic, 1, ros_save_occ_map_Callback);
  unknown_goal_status = n.subscribe(goal_status_topic, 1, ros_goal_status_Callback);
  unknown_pose = n.subscribe(pose_topic, 1, ros_set_goal_CallBack);
  ros::spin();
