...
void save_map_simple(pioneer3at::OccMap map, std::string robot)
{
  ...
  std::string mapdatafile = package + "/maps" + robot + "map.pgm";
...
void save_map_pose(pioneer3at::OccMap map, mapPose m_pose, std::string robot, int count)
{
  ...
  char sysCall[512];
  std::string package = ros::package::getPath("pioneer3at");
  std::string mapdatafile = package + "/maps" + robot + "pose" + boost::to_string(count);
