...
void ros_other_robot_pose_Callback(pioneer3at::poses aux_poses){
    ...
    if(sqrt(pow((x_mine - posesWave.x),2) - pow((y_mine - posesWave.y),2)) > 100 && timer==1){
        timer=0;
        posesWave.dist = dist;
        posesWave.x_other = -1;
        posesWave.y_other = -1;
...
int main( int argc, char* argv[] )
{
    ...
    frontier_other_robot_pose = n.subscribe(other_robot_pose_topic, 10, ros_other_robot_pose_Callback);
